...
static uint16_t ledMatrix[5];


static uint16_t ledMatrix2[5];
...
void updateLedMatrices(uint8_t* payload){
    for (uint8_t i = 0; i < 5; i++) {
        ...
        ledMatrix2[i] = ((uint16_t)payload[5*2 + i * 2 + 1] << 8) | payload[10 + i * 2];
    }
}
...
void setLEDToRed(uint8_t row, uint8_t column) {
    ...
    ledMatrix[row] |= (1 << (column * 3 + 2));
    ledMatrix[row] |= (1 << (column * 3 + 1));
    ledMatrix[row] &= ~(1 << (column * 3));
}
...
void initLEDMatrixBlue_Matrix2(void) {
    for (uint8_t i = 0; i < 5; i++) {
        ledMatrix2[i] = 0xFFFF;
        for (uint8_t j = 0; j < 5; j++) {
            ledMatrix2[i] &= ~(1 << (j * 3 + 1));
            ledMatrix2[i] |= (1 << (j * 3));
            ledMatrix2[i] |= (1 << (j * 3 + 2));
        }
    }
}
...
void setLEDToRed_Matrix2(uint8_t row, uint8_t column) {
    if (row >= 5 || column >= 5) return;
    ledMatrix2[row] |= (1 << (column * 3 + 1));
    ledMatrix2[row] |= (1 << (column * 3 + 2));
    ledMatrix2[row] &= ~(1 << (column * 3));
}
